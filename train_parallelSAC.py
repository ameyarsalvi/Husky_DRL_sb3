...
import sys
sys.path.insert(0,"/scratch/asalvi/HuskyVS/train/HuskyCP-gym")
import huskyCP_gym
...
tmp_path = '/zfs/deeprl/asalvi_backup/SAC/bsln/'
variant = 'bsln'
# set up logger
...
def make_env(env_id, rank, seed=0):
    ...
    def _init():
        ...
        seed = 1 + rank
        track_vel = 0.75
        env = gym.make(env_id, port = port_no,seed = seed, track_vel = track_vel)
        #env.seed(seed + rank)
        return env
...
if __name__ == '__main__':
    ...
    model = SAC("CnnPolicy", env,learning_rate=0.00001, buffer_size=100000, batch_size=512, tau=0.05, learning_starts= 500, gamma=0.99, train_freq=(1,"step"),
                action_noise=None,sde_sample_freq=16, 
                policy_kwargs=dict(normalize_images=False, log_std_init=-1.0, activation_fn=nn.ReLU, net_arch=dict(pi=[64], qf=[64])), 
