...
class HuskyCPEnvPath(Env):
    def __init__(self,port,seed,track_vel):
        ...
        self.path1 = genfromtxt('/home/asalvi/code_workspace/Husky_CS_SB3/Paths/InfPath/ArcPath1.csv', delimiter=',')
        self.path2 = genfromtxt('/home/asalvi/code_workspace/Husky_CS_SB3/Paths/InfPath/ArcPath2.csv', delimiter=',')
        self.path3 = genfromtxt('/home/asalvi/code_workspace/Husky_CS_SB3/Paths/InfPath/ArcPath3.csv', delimiter=',')
        self.path4 = genfromtxt('/home/asalvi/code_workspace/Husky_CS_SB3/Paths/InfPath/ArcPath4.csv', delimiter=',')
        self.path5 = genfromtxt('/home/asalvi/code_workspace/Husky_CS_SB3/Paths/InfPath/ArcPath5.csv', delimiter=',')
    ...
    def step(self,action):
        ...
        self.sim.setJointPosition(self.CameraJoint, self.camActRel.item())
        ...
        #Camera Joint Velocity reduction terms
        cam_jt_vel = self.sim.getJointVelocity(self.CameraJoint)
        cam_jt_vel = np.abs(cam_jt_vel)
        cam_jt_vel = np.clip(cam_jt_vel,0,3.14)
        norm_camjt_vel = (cam_jt_vel)/3.14

        # Total reward
        reward = 0*(1 - norm_err_track)**2 + (1 - norm_err_vel)**2 +(1- norm_err_eff)**2 + (1- norm_err_path)**2 + 0*(1-norm_camjt_vel)**2
        ...
        if self.episode_length ==0 or np.abs(self.path_track_err)>1 or reset == 1:
            done = True
        else:
            done = False
    ...
    def reset(self, seed=None):
        ...
        rand_spawn = np.random.randint(1, 5, 1, dtype=int)
        #rand_spawn = 5
        if rand_spawn == 1:
            # Create a rotation object from Euler angles specifying axes of rotation
            rot = Rotation.from_euler('xyz', [0, 0,  self.path1[0,2]], degrees=False)
            rot_quat = rot.as_quat()
            pose = [self.path1[0,0],self.path1[0,1],0.325,rot_quat[0],0,0,rot_quat[3]]
            self.path_selector = 1

        elif rand_spawn == 2:
            rot = Rotation.from_euler('xyz', [0, 0, self.path2[0,2]], degrees=False)
            rot_quat = rot.as_quat()
            pose = [self.path2[0,0],self.path2[0,1],0.325,rot_quat[0],0,0,rot_quat[3]]
            self.path_selector = 2

        elif rand_spawn == 3:
            rot = Rotation.from_euler('xyz', [0, 0, self.path3[0,2]], degrees=False)
            rot_quat = rot.as_quat()
            pose = [self.path3[0,0],self.path3[0,1],0.325,rot_quat[0],0,0,rot_quat[3]]
            self.path_selector = 3

        elif rand_spawn == 4:
            rot = Rotation.from_euler('xyz', [0, 0, self.path4[0,2]], degrees=False)
            rot_quat = rot.as_quat()
            pose = [self.path4[0,0],self.path4[0,1],0.325,rot_quat[0],0,0,rot_quat[3]]
            self.path_selector = 4

        #elif rand_spawn == 5:
        #    rot = Rotation.from_euler('xyz', [0, 0, math.pi/2 + self.path5[0,2]], degrees=False)
        #    rot_quat = rot.as_quat()
        #    pose = [self.path5[0,0],self.path5[0,1],0.325,rot_quat[0],0,0,rot_quat[3]]
    ...
    def camPD(self):
        ...
        curCam = self.sim.setJointPosition(self.CameraJoint)   
        self.camErr = self.camActSet.item()*math.pi/180 - curCam
        camErrDot = (self.camErr-self.camErrPrv)/0.05
        self.camActRel = camP*self.camErr + camD*camErrDot

        self.camErrPrv = self.camErr
