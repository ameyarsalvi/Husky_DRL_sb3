...
class HuskyCPEnvPathFrenC(Env):
    ...
    def arc_length(self):
        ...
        #####  CameraPose Error
        self.currentCam_pose = self.sim.getObjectPose(self.CameraJoint, self.sim.handle_world)
        currentCamQ = np.array([[self.currentCam_pose[3],self.currentCam_pose[4],self.currentCam_pose[5],self.currentCam_pose[6]]])
        self.Cam_pose_track_err = 1- np.dot(des_poseQ,np.transpose(currentCamQ))
    ...
    def GenControl(self,action):
        ...
        #camSet = (3.14/2)*action[2]
        #camSet_ = 0.06 * camSet + (1 - k) * self.CamPositionOld 
        #camSet_ = (camSet- self.CamPositionOld)*0.00005
        #e^{\frac{-1}{x+0.2}}
        #self.cam_err_old = (np.abs(self.CamPositionOld - CamPosition))/3.14

        camSet = 0.01*action[2]
        camSet_ = np.clip(self.CamPositionOld + camSet.item(),-3.14/2,3.14/2)


        self.sim.setJointPosition(self.CameraJoint, camSet_)
        
        CamPosition = self.sim.getJointPosition(self.CameraJoint)
        self.CamPositionOld = CamPosition


        #self.img_div = CamPosition/(0.5*3.14)
        self.img_div = 1
    ...
    def getReward(self):
     ...
     # Calculate Reward
        ...
        # Total reward
        #self.rew = (1 - norm_err_vel)**2 +(1- norm_err_eff)**2 + (1- norm_err_path)**2 + (1 - self.pose_track_err)**2 + (1 - self.Cam_pose_track_err) + (1 - norm_kappa)**2 + (1-self.cam_err_old)**2
        self.rew = (1 - norm_err_vel)**2 +(1- norm_err_eff)**2 + (1- norm_err_path)**2 + (1 - self.pose_track_err)**2 + (1 - norm_err_track) + (1 - norm_kappa)**2
